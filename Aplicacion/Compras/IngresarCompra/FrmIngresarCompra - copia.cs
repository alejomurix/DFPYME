...
{
    public partial class FrmIngresarCompra2 : Form
    {
        ...
        private void FrmIngresarCompra_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyData)
            {
                case Keys.F2:
                    {
                        this.btnBuscarProveedor_Click(this.btnBuscarProveedor, new EventArgs());
                        break;
                    }
                case Keys.F3:
                    {
                        this.btnBuscarArticulo_Click(this.btnBuscarArticulo, new EventArgs());
                        break;
                    }
                case Keys.F4:
                    {
        ...
        private void tsGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (MiProveedor != null)
            {
                ...
                {
                    if (codigoProveedor && NumFacturaMatch && FechaMatch && DescuentoMatch && AjusteMatch)
                    {
        ...
        private void cbRemision_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    txtNumeroFactura.Text = AppConfiguracion.ValorSeccion("documento");
                    //chkIva.Checked = true;
        ...
        private void dgvListaArticulos_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dgvListaArticulos.RowCount != 0)
                {
                    ...
                    lblValorCostoAsig.Text = UseObject.InsertSeparatorMil(miBussinesProducto.PrecioDeCosto(producto.CodigoInternoProducto).ToString());
                    ...
                    {
                        costo = Convert.ToInt32(UseObject.RemoveSeparatorMil(txtCosto.Text) / (1 + (producto.ValorIva)));
                    }
                    else
                    {
                        costo = Convert.ToInt32(UseObject.RemoveSeparatorMil(txtCosto.Text));
        ...
        private void btnCambiarRetencion_Click(object sender, EventArgs e)
        {
            ...
            if (rta.Equals(DialogResult.Yes))
            {
                ...
                /*TasaRete = query.Tarifa;
                BaseRete = query.CifraPesos;*/
        ...
        private void CargarFactura()
        {
            ...
            foreach (DataRow row in miTabla.Rows)
            {
                ...
                producto.Producto.ValorCosto = Convert.ToDouble(Convert.ToInt32(row["TotalMasIva"]));
        ...
        private void FinishProcessGuardado()
        {
            ...
            this.Enabled = true;*/
            var tipoFactura = Convert.ToInt32(cbRemision.SelectedValue);
           /* if (SuccessGuardado)
            {*/
                OptionPane.MessageSuccess("Las operaciones se realizaron correctamente.");
                if ((int)cbFormaPago.SelectedValue == 1) //  Pago de contado de la factura
                {
                    var msn = "";
                    if (EsFactura)
                    {
                        msn = "Factura de Proveedor";
                    }
                    else
                    {
                        msn = "Remisión de Proveedor";
                    }
                    /*DialogResult rta = MessageBox.Show("¿Desea ingresar el pago a la " + msn + "?", "Factura Proveedor",
                        MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    /*if (rta.Equals(DialogResult.Yes))
                    {
                        var pagoFactura = new Pago.FrmCancelarCompra();
                        pagoFactura.NitProveedor = MiProveedor.NitProveedor;
                        pagoFactura.EsFactura = EsFactura;
                        pagoFactura.IdFactura = miFactura.Id.ToString();
                        pagoFactura.NumeroFactura = txtNumeroFactura.Text;
                        pagoFactura.AplicaRetencion = AplicaRete;
                        pagoFactura.Retencion = MiRetencion;
                        if (tipoFactura.Equals(3))
                        {
                            pagoFactura.DocEquivalente = true;
                        }
                        if (AplicaRete)
                        {
                            pagoFactura.txtTotal.Text = UseObject.InsertSeparatorMil((UseObject.RemoveSeparatorMil(txtRetencion.Text) +
                                                                                        UseObject.RemoveSeparatorMil(txtTotal.Text)
                                                                                      ).ToString());
                        }
                        else
                        {
                            pagoFactura.txtTotal.Text = UseObject.InsertSeparatorMil
                                                        (Convert.ToInt32(UseObject.RemoveSeparatorMil(txtTotal.Text)).ToString());
                        }
                    ...
                    {*/
                        if (tipoFactura.Equals(3))
                        {
                            DialogResult rta = MessageBox.Show("¿Desea imprimir el Documento Equivalente?", "Factura Proveedor",
                            MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                            if (rta.Equals(DialogResult.Yes))
                            {
                                PrintDocumento();
            ...
            else
            {
                ...
                if (rta.Equals(DialogResult.Yes))
                {
                    if (!FormProveedor)
                    {
                        KeyProveedorPress = true;
                        formProveedor = new Proveedor.frmProveedor();
                        formProveedor.txtCodigoInterno.Text = nit;
                        formProveedor.Extension = true;
                        FormProveedor = true;
                        formProveedor.MdiParent = this.MdiParent;
                        formProveedor.Show();
                        ...
                        foreach (DataRow row in copyMiTabla.Rows)
                        {
                            ...
                            if (MiProveedor.IdRegimen == 1)
                            {
                                ...
                                InformacionTributaria(UseObject.RemoveCharacter(row_["Iva"].ToString(), '%'),
                                     row_["Cantidad"].ToString(), (string)row_["ValorUnitario"].ToString());
                            }
                            else
                            {
                                row_["Iva"] = 0 + "%";
        ...
        private void PrintDocumento()
        {
            try
            {
                ...
                frmPrintDocument.fecha = miFactura.FechaIngreso;
