...
{
    public partial class FrmConfConversion : Form
    {
        ...
        private void tsBtnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DialogResult rta = MessageBox.Show("¿Esta seguro(a) de guardar los datos?", "Conversión de producto",
                        ...
                        if (CodigoOrString(this.txtProductoP.Text))
                        {
                            if (ExisteProducto(this.txtProductoP.Text))
                            {
                                if (!this.miBussinesConversion.ExisteConversion(this.txtProductoP.Text))
                                {
                                    ...
                                    pRow["Venta"] = this.MiProducto.ValorVentaProducto;
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.MiProducto != null)
                {
                    ...
                    {
                        DialogResult rta = MessageBox.Show("¿Esta seguro(a) de agregar el registro?", "Conversión producto",
                            MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (rta.Equals(DialogResult.Yes))
                        {
                            this.miError.SetError(this.panelAgregar, null);
                            var detalle = new DetalleConversion();
                            detalle.IdConversion = Convert.ToInt32(this.dgvProducto.CurrentRow.Cells["IdConsulta"].Value);
