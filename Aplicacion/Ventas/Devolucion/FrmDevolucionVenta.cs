...
namespace Aplicacion.Ventas.Devolucion
{
    public partial class FrmDevolucionVenta : Form
    {
        ...
        private void FrmDevolucion_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                btnFactura_Click(this.btnFactura, new EventArgs());
            }
            else
            {
                if (e.KeyData.Equals(Keys.F4))
                {
                    tsBtnBuscarProducto_Click(this.tsBtnBuscarProducto, new EventArgs());
                }
                else
                {
                    if (e.KeyData.Equals(Keys.F5))
                    {
                        tsBtnCambiarPrecio_Click(this.tsBtnCambiarPrecio, new EventArgs());
                    }
                    else
                    {
                        if (e.KeyData.Equals(Keys.F6))
                        {
                            tsBtnRetiro_Click(this.tsBtnRetiro, new EventArgs());
                        }
                        else
                        {
                            if (e.KeyData.Equals(Keys.F7))
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    tsBtnSalir_Click(this.tsBtnSalir, new EventArgs());
                                }
                            }
                        }
                    }
        ...
        private void tsBtnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (rta.Equals(DialogResult.Yes))
            {
                ...
                try
                {
                    ...
                    {
                        GuardaSeguimientoBono(GuardarBono(Factura.Proveedor.NitProveedor, true), true);
                    ...
                    else//no se cobra y genera saldo, Cliente requerido
                    {
                        ...
                        {
                            GuardaSeguimientoBono(GuardarBono(Factura.Proveedor.NitProveedor, false), false);
        ...
        private void chkIncluyeIva_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar.Equals((char)Keys.Enter))
            {
                txtCantidad.Focus();
        ...
        private void txtCodigoArticulo_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar.Equals((char)Keys.Enter))
            {
                ...
                {
                    //if (ProductoDeFactura(txtCodigoArticulo.Text, Factura.Numero))
                    //{
        ...
        private bool CargarFactura(string numero)
        {
            try
            {
                ...
                if (tabla.Rows.Count != 0)
                {
                    ...
                    Detalle = miBussinesFactura.Detalles(numero);
        ...
        /// <returns></returns>
        public bool ProductoDeFactura(string factura, DetalleProducto dProducto)
        {
            try
            {
        ...
        private void EstructurarConsulta()
        {
            ...
            if (MiProducto != null)
            {
                ...
                LimpiarCampos();
                btnTallaYcolor.Enabled = false;
                /*if (ProductoDeFactura(Factura.Numero, detalle))
                {
        ...
        private void GuardaSeguimientoBono(Bono bono, bool saldo)
        {
            try
            {
                if (!saldo)
