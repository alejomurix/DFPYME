...
namespace Aplicacion.Ventas.Factura.Abonos
{
    public partial class FrmCancelarVenta : Form
    {
        ...
        private void FrmCancelarVenta_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.F6)
            {
                ...
                txtTarjeta_Validating(txtTarjeta, new CancelEventArgs(false));
                 ...
                 //{
                     if (AbonoMatch && Efectivo  && Tarjeta)
                     {
                             ...
                             if (!UseObject.RemoveSeparatorMil(this.txtAbono.Text).Equals(0))
                             {
                                 ...
                                 if (rta == DialogResult.Yes)
                                 {
                                     ...
                                     try
                                     {
                                         ...
                                         {
                                             // EDICION CODIGO NUEVO 30-08-2016

                                             if (Convert.ToBoolean(AppConfiguracion.ValorSeccion("print_termal_80mm")))
                                             {
                                                 PrintIngresoPos(Numero, ingreso.Saldo);
                                             }
                                             else
                                             {
                                                 PrintIngresoPos50mm(Numero, ingreso.Saldo);
                                             }
                                             // END EDICION 30-08-2016
        ...
        {
            // la forma de pago tarjeta es usada como forma de pago Transaccion
        ...
        private void PrintIngresoPos(string numero, int saldo)
        {
            try
            {
                ...
                if (rta.Equals(DialogResult.Yes))
                {
                    ...
                    if (EsVenta)
                    {
                        ...
                        printTicket.empresaRow = empresaRow;
                        printTicket.Usuario = usuario;
                        printTicket.anticipo = new Saldo { Fecha = Convert.ToDateTime(ingresoRow["fecha"]), Numero = numero };
                        printTicket.NoCaja = caja.Numero.ToString();
                        printTicket.Cliente = cliente;
                        printTicket.DetalleRbo = ingresoRow["Concepto"].ToString();
                        printTicket.ValorRbo = UseObject.InsertSeparatorMil(ingresoRow["Valor"].ToString());
                        printTicket.tPagos = tPagos;
                        printTicket.saldoAbono = UseObject.InsertSeparatorMil(saldo.ToString());
                        printTicket.ImprimirRboAbono();
                    }
                    else
                    {
                        ...
                        {
                            // EDICION CODIGO NUEVO 30-08-2016
                            valor = UseObject.InsertSeparatorMil(saldo.ToString());
                            ticket.AddHeaderLine("");
...
/*
        ...
        private void PrintIngresoPos50mm(string numero, int saldo)
        {
            try
            {
                DialogResult rta = MessageBox.Show("¿Desea imprimir el comprobante de ingreso?", "Abono a Factura",
                ...
                if (rta.Equals(DialogResult.Yes))
                {
                    ...
                    ticket.AddHeaderLine("---------------------------");
                    ticket.AddHeaderLine("SALDO: $        " + UseObject.InsertSeparatorMil(saldo.ToString()));
                    ticket.AddHeaderLine("---------------------------");
                    ticket.AddHeaderLine("");
                    ticket.AddHeaderLine("");
