    ...
    public partial class FrmFacturaVenta : Form
    {
        ...
        private void FrmFacturaVenta_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.FacturaPos)
                {
                    this.tsBtnDevolucion.Visible = false;
                }
                ...
                if (ClienteMatch && LimiteMatch)
                {
                    ...
                    if (IdEstado != 3)
                    {
                        ...
                        if (rta.Equals(DialogResult.Yes))
                        {
                            ...
                            {
                                Print(p.Id, p.Numero, p.Descuento, p.Cliente, p.Contado, IdEstado);
                            }
                        }
                    }
        ...
        private void Print(int id, string numero, bool descto, string cliente, bool contado, int idEstado)
        {
            ...
            try
            {
                ...
                print.DsEmpresa = dsEmpresa.Tables[0];
                print.DsFactura = dsFactura.Tables[0];
                print.DsUsuario = dsUsuario.Tables[0];
                print.DsCliente = dsCliente.Tables[0];
                print.DsDetalle = dsDetalle.Tables[0];
                print.Numero = numero;
                if (empresa.Regimen.IdRegimen.Equals(1))
                {
                    print.ResDian = dsDian.Tables["TDian"].AsEnumerable().First()["print_dian"].ToString();
                }
                else
                {
                    print.ResDian = " ";
                ...
                print.Retencion = miBussinesRetencion.ValorRetencionAventa(numero, descto);
                print.ReportPath = AppConfiguracion.ValorSeccion("report") + @"\reports\FacturaVenta.rdlc";
                print.Print();*/
            }
            catch (Exception ex)
            {
        ...
        private void PrintDetail(DataTable tDetalle, Ticket ticket, List<ProductoFacturaProveedor> lstProducto)
        {
            ...
            ticket.AddHeaderLine("CANT  DESCRIPCIÓN             TOTAL");
            ticket.AddHeaderLine("");
            foreach (DataRow dRow in tDetalle.Rows)
            {
                if (this.DescuentoMarca)
