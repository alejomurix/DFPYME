...
namespace Aplicacion.Ventas.Factura.PagosV2
{
    public partial class FrmCancelarVentaV2 : Form
    {
        private bool Venta = true;

        private bool Abono = false;

        private bool EsVenta = true;

        public List<FormaPago> Formas { set; get; }
            ...
            if (Efectivo <= Total)
            {
                if (Efectivo != 0)
                {
                    Formas.Add(new DTO.Clases.FormaPago
                    {
                        ...
                        IdFormaPago = 1,
                        Valor = Convert.ToInt64(Efectivo),
                        Pago = Convert.ToInt64(Efectivo)
                    });
                }
                if (Cheque <= (Total - Efectivo))
                {
                    if (Cheque != 0)
                    {
                        Formas.Add(new DTO.Clases.FormaPago
                        {
                            IdEgreso = Convert.ToInt32(AppConfiguracion.ValorSeccion("idturno")),
                            IdFormaPago = 2,
                            Valor = Convert.ToInt64(Cheque),
                            Pago = Convert.ToInt64(Cheque)
                        });
                    }
                    if (Tarjeta <= (Total - (Efectivo + Cheque)))
                    {
                        if (Tarjeta != 0)
                        {
                            Formas.Add(new DTO.Clases.FormaPago
                            {
                                IdEgreso = Convert.ToInt32(AppConfiguracion.ValorSeccion("idturno")),
                                IdFormaPago = 4,
                                Valor = Convert.ToInt64(Tarjeta),
                                Pago = Convert.ToInt64(Tarjeta)
                            });
                        }
                    }
                    else
                    {
                        Formas.Add(new DTO.Clases.FormaPago
                        {
                            IdEgreso = Convert.ToInt32(AppConfiguracion.ValorSeccion("idturno")),
                            IdFormaPago = 4,
                            Valor = Convert.ToInt64(Total - (Efectivo + Cheque)),
                            Pago = Convert.ToInt64(Tarjeta)
                        });
                    }
                }
                else
                {
                    Formas.Add(new DTO.Clases.FormaPago
                    {
                        IdEgreso = Convert.ToInt32(AppConfiguracion.ValorSeccion("idturno")),
                        IdFormaPago = 2,
                        Valor = Convert.ToInt64(Total - Efectivo),
                        Pago = Convert.ToInt64(Cheque)
                    });
                }
            }
