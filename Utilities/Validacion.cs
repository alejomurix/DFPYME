...
namespace Utilities
{
    ...
    public class Validacion
    {
        ...
        public enum TipoValidacion 
        {
            ...
            /// <summary>
            /// Indica que la validacion se hara a solo letras y numeros y que pueden tener guion.
            /// </summary>
            LetrasGuionNumeros,

            /// <summary>
            /// Indica que la validación se hará a letras con números y caracteres (-_.) sin espacios.
            /// </summary>
        ...
        private const string numeroPunto_ = @"^([0-9])+.[0-9]+$";
        ...
        private const string letrasGuionNumeros = "^[a-zA-Z-0-9]+$";

        private const string letrasNumerosCaracteres = @"^[a-zA-Z-0-9\-\\_\.]+$";
         ...
         (Validacion.TipoValidacion tipoValidacion, Control control, ErrorProvider error, string mensaje)
        {
            ...
            switch (tipoValidacion)
            {
                ...
                case TipoValidacion.LetrasGuionNumeros:
                    {
                        resultado = Regex.IsMatch(control.Text, letrasGuionNumeros);
                        break;
                    }
                case TipoValidacion.LetrasNumerosCaracteres:
                    {
                        resultado = Regex.IsMatch(control.Text, letrasNumerosCaracteres);
                        break;
                    }
        ...
        /// <returns></returns>
        public bool ValidarNumeroDecima(string numero)
        {
            return Regex.IsMatch(numero, numerosPunto);//numeroPunto_);
        }

        public static bool ValidateDouble(string val)
        {
            var pass = false;
            try
            {
